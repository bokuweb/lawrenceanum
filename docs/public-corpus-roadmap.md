...
- 法務省の現行審議会一覧から会議 URL を辿り、議題・議事概要・議事録 TXT/PDF・配布資料を
  取得する。会議 HTML と添付原本は内容アドレス、会議状態は意味のある変更単位で履歴化し、
  R2 へ日次永続化する。直近20会議を日次監視し、手動指定で全履歴も収集できる。
- LLM Wiki / OKF はこの正規化コーパスの**下流の派生物**とする。raw と安定 JSON を正本にし、
  index/fetch 時に LLM を呼ばない原則は維持する。

次の収集優先順位:

1. 審議会の対象府省を内閣府・厚生労働省・国土交通省へ拡張し、法令リンクを生成する。
